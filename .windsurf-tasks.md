...
After you generate this task list, here is a prompt to use in Windsurf agent to kick this off (might be useful to put at the end of your windsurf-rules file as well?)
Probably helpful to just @include the windsurf-tasks.md file as well. 
``` txt
Go through each task in the .windsurf-tasks file. After you complete each task, update the file to check off any task. Run builds and commits after each task. Continue with each task until you have checked off each one. After each story, do not take a screenshot. If you need more detail about a task, you can gather relevant files and pass the FULL file to the research agent.
```
